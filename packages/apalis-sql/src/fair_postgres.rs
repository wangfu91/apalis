...
// #[derive(Debug)]
pub struct FairPostgresStorage<T, C = JsonCodec<serde_json::Value>>
where
    C: Codec,
{
    pool: PgPool,
...
impl<T, C: Codec> Clone for FairPostgresStorage<T, C> {
    fn clone(&self) -> Self {
        FairPostgresStorage {
...
impl<T, C: Codec> fmt::Debug for FairPostgresStorage<T, C> {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        f.debug_struct("FairPostgresStorage")
...
impl<T, C> Backend<Request<T, SqlContext>> for FairPostgresStorage<T, C>
where
    ...
    C::Error: std::error::Error + 'static + Send + Sync,
{
    ...
    type Layer = AckLayer<FairPostgresStorage<T, C>, T, SqlContext, C>;
    ...
    fn poll(mut self, worker: &Worker<Context>) -> Poller<Self::Stream, Self::Layer> {
        ...
        let heartbeat = async move {
            ...
            >(
                storage: &mut FairPostgresStorage<T, C>,
                worker: &WorkerId,
...
impl<T, C: Codec> FairPostgresStorage<T, C> {
    /// Expose the codec
...
impl PgListen {
    ...
    ///
    /// Maintaining a connection can be expensive, its encouraged you only create one [PgListen] and share it with multiple [FairPostgresStorage]
    ...
    pub fn subscribe_with<T>(&mut self, storage: &mut FairPostgresStorage<T>) {
...
impl<T, C> FairPostgresStorage<T, C>
where
    ...
    C: Codec<Compact = Value>,
{
    async fn fetch_next(
        ...
        let fetch_query = "Select * from apalis.get_fair_jobs($1, $2, $3);";
...
impl<Req, C> Storage for FairPostgresStorage<Req, C>
where
...
impl<T, Res, C> Ack<T, Res, C> for FairPostgresStorage<T, C>
where
    T: Sync + Send,
...
mod tests {
    ...
    sql_storage_tests!(setup::<Email>, FairPostgresStorage<Email>, Email);

    /// migrate DB and return a storage instance.
    async fn setup<T: Serialize + DeserializeOwned>() -> FairPostgresStorage<T> {
    ...
    async fn cleanup<T>(storage: &mut FairPostgresStorage<T>, worker_id: &WorkerId) {
        let mut tx = storage
            .pool
            .acquire()
            .await
    ...
    #[tokio::test]
    async fn test_fair_fetching_logic() {
        let mut storage: FairPostgresStorage<UserJob> = setup().await;

        // Push 3 jobs for user "user-1"
        for i in 0..3 {
            storage
                .push(UserJob {
                    identity: UserIdentity {
        ...
        let user_ids: HashSet<String> = jobs
            .iter()
            .map(|j| j.args.identity.user_id.clone())
            .collect();
        assert!(user_ids.contains("user-1"));
        assert!(user_ids.contains("user-2"));
        assert!(user_ids.contains("user-3"));
        ...
        let user_ids_2: HashSet<String> = jobs_2
            .iter()
            .map(|j| j.args.identity.user_id.clone())
            .collect();
        assert!(user_ids_2.contains("user-1"));
        assert!(user_ids_2.contains("user-2"));
        assert!(!user_ids_2.contains("user-3"));
    ...
    async fn test_fetch_with_different_job_type_returns_empty() {
        ...
        let mut storage_email: FairPostgresStorage<Email> = setup().await;
        ...
        let mut storage_sms: FairPostgresStorage<Email> =
            FairPostgresStorage::new_with_config(pool, sms_config);
